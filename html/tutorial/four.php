...
void touchRstZoneF( Zone zone){
	zone.rst( true, true, false, true);
		...
		<p>
			<code>Zone.getTouches()</code> is the first step. It returns an array containing all the touches that are currently assigned to that zone. <code>Touch.getX()</code> and <code>Touch.getY()</code> can be used to get the current position of a touch. <code>Touch.getLocalX( Zone)</code> and <code>Touch.getLocalY( Zone)</code> can be used to get the current position of a touch, relative to the zone passed to it. <code>Touch.getPathPoints()</code> returns an array containing the recorded points of the touch's path. If timepoints are also needed, <code>Touch.getPath()</code> returns an array of <a href="http://tuio.org/api/java/TUIO/TuioPoint.html">TuioPoint</a>s. The TuioPoint class has the function <code>getTuioTime()</code> for getting the time of the point. Comparing that time to <code>TuioTime.getSessionTime()</code> can tell you how long ago that point is in time.
		</p>
		<p>
