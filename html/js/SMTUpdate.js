function updateDate() {
    ...
    $.get('proxy.php?url=' + uri, function (data) {
        var xml = $.parseXML(data),
            entry = $(xml).find("entry")[0],
            txtDate = $(entry).find("updated").text().substring(0, 10),
            now = new Date(),
            entryDate = new Date(),
            daysPast;

        entryDate.setFullYear(txtDate.substring(0, 4), (parseInt(txtDate.substring(5, 7)) - 1), txtDate.substring(8, 10));
